...
class InternVisionEmbeddings(nn.Module):
    ...
    def forward_adapter(self, x):
        ...
        # print(f"Patch embeds shape: {patch_embeds.shape}")
        # print(f"Position embedding shape: {self.position_embedding.shape}")
        # print(f"H={H}, W={W}, n={n}")
        # print(f"Expected patches: {(H*W)}")
        
        # 使用 _get_pos_embed 来正确调整位置嵌入尺寸
        pos_embed = self._get_pos_embed(self.position_embedding[:, 1:, :], H, W)
        x = patch_embeds + pos_embed
    ...
    def forward(self, x):
        ...
        # print(target_dtype)
        patch_embeds = self.patch_embedding(x).to(target_dtype)  # shape = [*, channel, width, height]
